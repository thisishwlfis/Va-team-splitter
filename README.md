...
## 임시 참가자 추가 (1회성)
"고정 멤버"가 아니라 이번 한 판만 참여하는 게스트가 있을 경우, 메인 화면(`index.html`)
1단계 참가자 선택 화면에서 **"+ 임시 참가자 추가"** 버튼으로 닉네임#태그/실명/티어를
입력해 바로 목록에 추가하고 선택할 수 있습니다. 이렇게 추가한 참가자는 **이 브라우저의
현재 화면(메모리)에서만 존재**하며 `data/players.json`에는 전혀 저장되지 않으므로,
새로고침하거나 다른 사람이 접속하면 사라집니다. 계속 쓸 고정 멤버는 `register.html`
(등록 신청 → 관리자 승인) 또는 `admin.html`을 이용해 정식으로 등록하세요.
